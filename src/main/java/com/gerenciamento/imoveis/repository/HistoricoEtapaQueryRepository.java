...
public class HistoricoEtapaQueryRepository {
    ...
    public List<HistoricoDuracaoDto> findDurationsByImovelId(String imovelId) {
        String sql = "WITH events AS ("
                ...
                + " durations AS ("
                + " SELECT imovel_id, etapa, data_alteracao AS started_at, COALESCE(next_ts, now()) AS ended_at,"
                + " EXTRACT(epoch FROM (COALESCE(next_ts, now()) - data_alteracao))/86400.0 AS days"
                + " FROM events)"
    ...
    public List<HistoricoDuracaoAggregateDto> findAggregatedByImovelId(String imovelId) {
        String sql = "WITH events AS ("
                ...
                + " FROM public.historico_etapas WHERE imovel_id = ?),"
                + " durations AS ("
                + " SELECT imovel_id, etapa, EXTRACT(epoch FROM (COALESCE(next_ts, now()) - data_alteracao))/86400.0 AS days"
                + " FROM events)"
